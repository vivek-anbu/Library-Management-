...
public class ServiceBook implements BookService {

	    public static final String RED="\u001B[31m";         // Color pattern Codes
	    ...
	    Validator validator=new Validator();         //Validator class
	    List<Book> books=new ArrayList<>();				//Arraylist for book
	    ArrayList<User>users=new ArrayList<>();				//Arraylist for users
	    @Override
		public void addUser() {
			
	    	String username=validator.ValidateUserame();
	    	String emailid=validator.ValidateEmail();
	    	String password=validator.ValidatePassword();
	    	User user=new User(username,emailid,password);
	    	users.add(user);
	    	System.out.println(GREEN+"Account Created Sucessfully!"+RESET);
		...
		public void login() {
			...
			for(User user:users)
			{
				...
				{
					System.out.println(BLUE+"Welcome "+user.getUsername()+RESET);
				}
				else
				{
					System.out.println(RED+"Please Create Account As NewUser"+RESET);
				}
			}
